...
Status: proven in `scratch/bitcannon` for three grammars, all byte-exact against their
`bitcannon` grammar counterparts.
...
```
  ...
  fsm_byte_level ████████████████████████████                      1.71
  bitcannon ds    █████████                                         0.58
  bitcannon cl100k████████                                          0.50
  bitcannon bl    ██████                                            0.41
```

The FSM spread across grammars is 71% (1.71 → 2.93); bitcannon's is 20%. **Flat cost across grammars
...
```
  text  ─────────────────────────────────────────────────────────────┐
    │                                                                │
    ▼                                                                │
  classify (bitcannon)           one Atom tag per byte                │
    │                                                                │
    ▼                                                                ▼
