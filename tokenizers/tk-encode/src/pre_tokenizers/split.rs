...
pub struct Split {
    ...
    pub invert: bool,
    /// Native `bitcannon` FSM for a recognized GPT regex (gpt2 / cl100k-Llama-3 / o200k), used on the
...
// SAFETY: both routes cut only at character boundaries of `text`. The native route is a `bitcannon`
...
unsafe impl pipeline::PreTokenizer for Split {
    fn pre_tokenize(
        ...
        // straight to the native bitcannon grammar. These regexes cover the whole input, so
...
mod tests {
    ...
    fn native_accepts_patterns_new_would_need_an_engine_for() {
        ...
        let gpt2 = Split::native(
            SplitPattern::Regex(bitcannon::regexes::GPT2.to_string()),
            SplitDelimiterBehavior::Isolated,
            false,
        )
        .unwrap();
